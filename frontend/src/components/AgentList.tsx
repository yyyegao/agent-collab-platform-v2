...
export const AgentList: React.FC = () => {
  ...
  const handleSaveAgent = (agent: AgentConfig) => {
    console.log('[AgentList] handleSaveAgent called, agent:', JSON.stringify(agent).substring(0, 200));
    const { agents: currentAgents } = useAppStore.getState();
    if (editingAgent) {
      // 更新
      setAgents(currentAgents.map((a: AgentConfig) => a.id === editingAgent.id ? { ...agent, id: editingAgent.id } : a));
      showToast('保存成功');
    } else {
      // 新建（本地生成 ID）
      const newAgent = { ...agent, id: `agent-${Date.now()}` };
      setAgents([...currentAgents, newAgent]);
      showToast('新建 Agent 成功');
    }
    setShowModal(false);
  ...
  const handleDeleteAgent = (id: string) => {
    if (confirm('确定要删除这个 Agent 吗？')) {
      const { agents: currentAgents } = useAppStore.getState();
      setAgents(currentAgents.filter((a: AgentConfig) => a.id !== id));
      showToast('删除成功');
      setShowModal(false);
      setEditingAgent(null);
    }
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <div className="shrink-0 px-4 pt-4 pb-2">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold text-txt-primary">Agent 列表</h2>
            <p className="text-sm text-txt-secondary">管理你的 Agent 团队</p>
          </div>
          <button onClick={handleAddAgent} className="w-10 h-10 rounded-xl bg-accent-orange text-white flex items-center justify-center hover:bg-orange-600 transition-all shadow-sm shrink-0">
                ...
                <div key={agent.id} className="flex items-center gap-2">
                  ...
                  <div className="flex gap-1 shrink-0">
                    <button
                      ...
                      className="w-8 h-8 rounded-lg bg-warm-100 text-txt-secondary flex items-center justify-center hover:bg-blue-50 hover:text-blue-600 transition-colors"
                      title="编辑"
                    >
                    ...
                    <button
                      ...
                      className="w-8 h-8 rounded-lg bg-warm-100 text-txt-secondary flex items-center justify-center hover:bg-red-50 hover:text-red-600 transition-colors"
                      title="删除"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {agents.length === 0 && (
          <div className="text-center py-12">
            <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-warm-100 flex items-center justify-center">
              <svg className="w-8 h-8 text-txt-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" /></svg>
            </div>
            <p className="text-txt-secondary mb-2">还没有 Agent</p>
            <p className="text-sm text-txt-muted mb-4">点击 + 添加你的第一个 Agent</p>
      ...
      {/* Toast 通知 */}
      {toast && (
        <div className={`fixed bottom-24 left-1/2 -translate-x-1/2 px-5 py-3 rounded-xl shadow-lg font-medium text-sm z-50 transition-all ${
          toast.type === 'success'
